...
def start(period):
    ...
    driver.get('https://ppg.service-now.com/financepulse?id=credit_accrual_report')
    ...
    def getScreenshot(path):
        img = ImageGrab.grab()
        img.save(path + "params_accrual.png")
        time.sleep(2)

    def change_country(countries):
        ...
        for country in countries:
            ...
            #clear the country options

            i+=1


    def download_form(): # to click apply and initiate download
    ...
    def moveAndRenameFile(path_from, file, path_to, country, type): #moving from download path(check by chrome) to generation folder
        try:
            what_to_replace = str(path_from) + str(file)
            #print(what_to_replace)
            where_to_replace = str(path_to) + str(country) + "/" + "accrual"+ str(type)
            #print(where_to_replace)
            #shutil.move(what_to_replace, where_to_replace)
            try:
                #path = os.path.join(path_to, str(country))
                path = path_to + str(country)
                print(path)
                os.mkdir(path)
            except:
...
def getDownloadPath(): #get download path from chrome setting 
    #SETUP THE BROWSER
